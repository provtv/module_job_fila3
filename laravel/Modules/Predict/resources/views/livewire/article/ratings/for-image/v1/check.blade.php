    ...
    <div class="block w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        <form wire:submit="save">
                ...
                <div class="w-full p-1 mx-auto rounded bg-neutral-1">
                    <div class="flex items-center justify-between px-2 py-4">
                        ...
                        @if($type == 'show')
                            <div class="">
                                <!-- Dropdown menu -->
                                @include('predict::livewire.article.ratings.for-image.v1.check.dropdown_menu')
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="px-4 mb-3">
                ...
                {{-- {{ dddx(['predict_uuid' => $this->article->uuid, 'rating_id' => $this->rating_id]) }} --}}
                {{-- {{ dddx([
                    get_defined_vars(),
                    $article_ratings,
                    array_keys($article_ratings),
                    $this->article->ratings()->where('user_id', null)
                            ->get()
                            ->pluck('id')
                            ->toArray()
                ...
                @livewire('prediction-information', [
                        // 'predict_uuid' => $this->article->uuid, 
                        'amount' => $this->import,
